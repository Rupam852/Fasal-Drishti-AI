...
export default function ScanManager({ scans = [], onSaveScanUpdate }) {
  ...
  const availableCrops = ['All', ...Array.from(new Set(scans.map(s => s.cropName).filter(Boolean)))];
  ...
  const exportCSV = () => {
    ...
    link.setAttribute('download', `fasal_drishti_live_scans_${Date.now()}.csv`);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Live inspection, clinical validation, and agronomist prescription override for Supabase scan records
            </p>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-xs px-3 py-1.5 rounded-xl bg-slate-800 text-emerald-400 font-mono font-bold border border-white/5">
              {scans.length} Live Scans
            </span>
            <button
              onClick={exportCSV}
      ...
      <div className="glass-panel p-5 rounded-2xl border border-white/10 shadow-xl overflow-hidden">
        {scans.length === 0 ? (
          <div className="p-12 text-center rounded-2xl bg-slate-900/40 border border-white/5 space-y-3">
            <div className="inline-flex items-center justify-center p-3 rounded-2xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              <Radio className="w-6 h-6 animate-pulse" />
            </div>
            <h4 className="text-sm font-bold text-white">No Crop Scans in Database Yet</h4>
            <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
              When a farmer uses the Fasal Drishti Android app to diagnose a crop disease, the scan image and analysis will be stored in Supabase table <code className="text-emerald-400 font-mono">public.scans</code> and stream here in real-time.
            </p>
          </div>
        ) : (
